...
def _upload_playlist_via_ssh(playlist_name: str, content: str, host: str) -> str:
    ...
    ssh.connect(
        ...
        key_filename=host_config.get(
            "identityfile", str([Path.home() / ".ssh" / "lan"][0])
        ),
    ...
    try:
        ...
        query_params = "&".join(
            [
                ...
                if value or key in ["limit", "max_distance"]
            ]
        )
        ...
        safe_tags = tags.replace(",", "").replace(" ", "_")[
            :20
        ]  # First 20 chars of tags without spaces and commas
        ...
        try:
            error = _upload_playlist_via_ssh(
                playlist_name, m3u_content, cfg.music.mpd.host
            )
            if error:
